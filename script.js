const durations = { reset: 3 * 60, focus: 25 * 60, break: 5 * 60 };
 ...
 // Storage schema v2:
 // {
 //   days: number,          // current streak (consecutive days)
 //   hours: number,         // total deep-work hours (all time)
 //   lastDate: "YYYY-MM-DD",
 //   todaySessions: number, // pomodoros finished today
 //   history: { [date]: count } // last 30 days for the bar
 // }
 function loadStats() {
   ...
   // migrate old format that lacked todaySessions / history
 ...
 if (stats.lastDate !== today && stats.lastDate !== "") {
   // If the last date was not yesterday either, the streak is already handled in recordCompletion
   // Just reset today's session counter for the new day display
 ...
 function updateStreak() {
   ...
   if (stats.lastDate === today) {
     // same day — streak already counted, just increment today's sessions
   ...
   for (const d of Object.keys(stats.history)) {
     if (d < cutoff) delete stats.history[d];
   }
   saveStats();
 }
 ...
 function render() {
   ...
   // ── Tree growth fix ──
   // Use a minimum scale of 0 so the tree genuinely starts small and grows visibly.
   // The CSS formula is: scale(calc(0.62 + var(--growth) * 0.055))
   // We clamp to 0..7 so at 7 sessions it reaches max.
   ...
   el.hoursCount.textContent = `${(stats.hours || 0).toFixed(1)} hrs`;
   if (el.todayPomodoros) {
     const tp = stats.todaySessions || 0;
     el.todayPomodoros.textContent = `${tp} session${tp === 1 ? "" : "s"}`;
   }
   // show flame on active streaks
   if (el.streakFlame) {
     el.streakFlame.style.opacity = stats.days >= 2 ? "1" : "0";
   }
   renderStreakBar();
 ...
 function recordCompletion() {
   ...
   // update completion banner with streak info
